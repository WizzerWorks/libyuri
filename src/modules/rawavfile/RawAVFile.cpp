            ...
            if (loop_ && !has_next_filename() && fmtctx_) {
                ...
                if (black_on_end_) {
                    ...
                    for (auto i: irange(video_streams_.size())) {
                        ...
                        const auto frame = core::RawVideoFrame::create_empty(core::raw_format::rgb24,
                                                                             stream.resolution);
                        std::fill(PLANE_RAW_DATA(frame, 0), PLANE_RAW_DATA(frame, 0) + PLANE_SIZE(frame, 0), 0);


                        push_frame(i, blank_converter_->convert_frame(frame, format_out_));
//            stream.
            }
        }
